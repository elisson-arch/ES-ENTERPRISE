...
const SidebarItem: React.FC<SidebarItemProps> = ({ icon: Icon, label, to, active, onClick }) => (
  ...
  >
    <Icon size={20} />
    <span className="font-bold text-xs uppercase tracking-tight whitespace-nowrap">{label}</span>
  </Link>
);
...
export const Navigation: React.FC<NavigationProps> = ({ 
  ...
  return (
    <>
      {/* Mobile Header Agile */}
      <div className="lg:hidden fixed top-0 left-0 right-0 h-16 bg-white border-b border-slate-200 z-[60] flex items-center justify-between px-6 shadow-sm">
      ...
      {/* Desktop Sidebar / Mobile Drawer Modernized */}
      <aside className={`
        fixed inset-y-0 left-0 z-[70] w-64 bg-white border-r border-slate-200 transform transition-transform duration-300 ease-in-out lg:translate-x-0
        ...
        <div className="flex flex-col h-full">
          <div className="p-8 border-b border-slate-50 flex items-center justify-between bg-slate-50/30">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Enterprise Ready</p>
              </div>
            </div>
          </div>
          ...
          <div className="px-6 py-6">
            <button 
              ...
              className="w-full flex items-center gap-3 px-5 py-3.5 bg-slate-50 border border-slate-200 rounded-[1.25rem] text-slate-400 text-[10px] font-black uppercase tracking-widest hover:bg-white hover:border-blue-300 hover:shadow-lg hover:shadow-blue-50 transition-all group"
            >
          ...
          <div className="p-6 border-t border-slate-50 space-y-3">
            ...
            <button 
              ...
              className="flex items-center gap-3 w-full px-5 py-3 text-slate-400 font-black text-[10px] uppercase tracking-widest hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all"
            >
              <LogOut size={18} />
      ...
      {/* Mobile Agile Bottom Bar */}
      <nav className="lg:hidden fixed bottom-0 left-0 right-0 h-20 bg-white/80 backdrop-blur-xl border-t border-slate-200 z-[60] flex items-center justify-around px-6 pb-4 pt-2 shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        ...
        <Link to="/funnel" className={`flex flex-col items-center gap-1.5 transition-all ${location.pathname === '/funnel' ? 'text-blue-600 scale-110' : 'text-slate-300'}`}>
          ...
          <span className="text-[8px] font-black uppercase tracking-tighter">Funil</span>
        </Link>
      ...
      {isOpen && (
        <div 
          className="fixed inset-0 bg-slate-900/40 backdrop-blur-md z-[65] lg:hidden animate-in fade-in duration-300"
          onClick={() => setIsOpen(false)}
        />
      )}
    </>
